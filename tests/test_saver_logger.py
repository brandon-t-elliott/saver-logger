...
class SaverLoggerTest(unittest.TestCase):
    ...
    def test_export_is_plain_csv_without_comment_lines(self):
        """The export is plain CSV: a column header then one line per row, with
        no leading metadata block, no footer, and no comment (#) or blank
        lines."""
        self.ext.log_data.append(list(SAMPLE_ROW))
        path = os.path.join(self.tmp, 'out.csv')
        self.assertTrue(self.ext._write_full_csv(path))

        lines = read_raw(path).splitlines()
        self.assertTrue(lines[0].startswith('Serial No,'),
                        'the first line must be the column header')
        for line in lines:
            self.assertFalse(line.startswith('#'),
                             'no comment/metadata lines: %r' % line)
            self.assertNotEqual(line, '', 'no blank lines')
    ...
    def test_export_writes_column_header_and_rows(self):
        self.ext.log_data.append(list(SAMPLE_ROW))
        path = os.path.join(self.tmp, 'out.csv')
        self.assertTrue(self.ext._write_full_csv(path))
    ...
    def test_auto_backup_uses_single_overwritten_file(self):
        ...
        _, rows = read_csv_rows(os.path.join(self.tmp, autosaves[0]))
        self.assertEqual(len(rows), 2, 'the single file holds both rows')
...
class EndToEndSessionTest(unittest.TestCase):
    ...
    def _assert_export_matches(self, export_path, master):
        ...
        lines = read_raw(export_path).splitlines()
        self.assertTrue(lines and lines[0].startswith('Serial No,'),
                        'the export must start with the column header')
        self.assertFalse(any(line.startswith('#') for line in lines),
                         'the export must be plain CSV with no comment lines')
